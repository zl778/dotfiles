...
## 9. 如果 SSH 私钥泄露了，分几步走：
	...
	ssh-keygen -t ed25519 -C "pistachio26@gmail.com"
	
	会生成 ~/.ssh/id_ed25519（新）和 id_ed25519.pub
	...
	ssh root@199.115.228.154 'echo "新公钥内容" >> ~/.ssh/authorized_keys'
	...
	把 Bitwarden 里的旧私钥替换成新的。
	
	5️⃣ 清理旧密钥
	
	bash
	
	删掉旧的 known_hosts 记录（可选）
	
	ssh-keygen -R 199.115.228.154
	
	整个流程大概 5 分钟搞定。 核心就是：
	
	- 生成新的
	
	- 告诉 VPS（改 authorized_keys）
