...
## Multi-Instance Memory: When Does Sharing Actually Matter?

A common question when running multiple Hermes instances (e.g. macOS + Windows + Wukong): if all bots are in the same Telegram group, can one bot "see" what the others have done without shared memory?

**No — and the distinction matters.**

| Capability | Telegram group context | Shared memory provider |
|------------|:----------------------:|:----------------------:|
| Current session sees ongoing conversation | ✅ Both bots in the group hear it | — |
| Cross-instance preferences ("user likes X") | ❌ Each bot must learn independently | ✅ Auto-shared |
| Project-specific facts from weeks ago | ❌ Outside any session's window | ✅ Persisted & retrieved |
| Avoid re-explaining context on the third bot | ❌ Each bot's first encounter starts cold | ✅ Carries forward |

**Mental model:**
- Group chat = real-time context bus (ephemeral, within one session)
- Memory = persistent knowledge (crosses sessions, accumulates)

**When shared memory is actually worth it:**
- You keep forgetting to tell Bot W what you already told Bot M
- Bot W routinely asks the same question Bot M learned two months ago
- You have stable preferences/conventions worth auto-distributing

**When it's overkill (skip for now):**
- Each bot has a clearly distinct purpose and rarely overlaps
- You're OK re-explaining once per bot
- The deployment cost (Docker / API fees) outweighs the friction saved

For a 3-bot setup where the **orchestrator** (e.g. macOS) already routes everything to its subordinates via SSH, the orchestrator can inject the relevant context on each delegation — so subordinates don't strictly need independent memory. Centralize the memory on the orchestrator, keep subordinates lean.