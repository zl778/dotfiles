...
When a Kanban task is a coding, refactor, or test task with clear acceptance criteria, you may optionally delegate the implementation to a Codex CLI lane while keeping Kanban lifecycle ownership yourself. See `references/codex-lane.md` for the full protocol.

### Ownership Rules (Codex Lane)

1. **Hermes owns the Kanban lifecycle.** Codex must never call `kanban_complete`, `kanban_block`, or any Hermes board CLI.
...
4. **Hermes owns safety.** If Codex changes safety boundaries, risk gates, or secrets handling, reject the lane.
5. **Hermes owns cleanup.** Kill stuck Codex processes and remove temporary worktrees.

### When to Use the Codex Lane

- The task is a coding, refactor, documentation, test, or mechanical migration task
...
- The worker lacks repo access, Codex auth, or time to reconcile
- The change touches secrets, credential stores, private user data, or production systems
- A small direct edit is faster and safer

For the full protocol (worktree setup, prompt construction, monitoring, reconciliation checklist, kanban_complete metadata schema), see `references/codex-lane.md`.
