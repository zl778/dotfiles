...
- Designing a master/worker Hermes setup with Telegram, cmux, desktop, and multiple worker instances
- Mapping a new worker (e.g. Win11 Hermes or WSL Hermes) into an existing Hermes dispatcher
- Updating or documenting the user's preferred role/instance/channel separation for multi-agent work
- Designing a master/worker Hermes setup with Telegram, cmux, desktop, and multiple worker instances
- Mapping a new worker (e.g. Win11 Hermes or WSL Hermes) into an existing Hermes dispatcher
- Updating or documenting the user's preferred role/instance/channel separation for multi-agent work
...
## Role / Instance / Channel Quick Model

Use this model when a user is comparing multiple Hermes installations, bots, or interfaces:

- **Role** = what the agent does (Research/Coding/Browser).
- **Instance** = where the agent runs (Mac Hermes, Win11 Hermes, WSL Hermes, wukong process).
- **Channel** = how the user talks to the system (Telegram, cmux, desktop app/Kanban).

Default rule: start with one master entry point and treat roles as logical responsibilities. Split into separate instances only when you need parallelism, isolation, different models/providers, or a distinct execution environment.

## Worker Registry Pattern

When the user adds a new worker (for example Win11 Hermes or WSL Hermes), do not treat it as a brand-new role class. Instead, capture it in a registry record with:
- name
- role(s)
- host/environment
...
The master agent should route by role first, then by environment, then by current load/availability.
...
See `references/hermes-multi-agent-orchestration.md` for the distilled Hermes-centric orchestration model, worker registry fields, task-intake template, and the role/instance/channel split.
...
- **Do not confuse role with instance.** Research/Coding/Browser are usually logical responsibilities, not automatically separate bots. Only split them into separate instances when parallelism, isolation, or environment differences justify it.
- **Do not confuse channel with architecture.** Telegram, cmux, and the desktop app are user interaction modes; they do not by themselves define the underlying agent hierarchy.
- **A Telegram group does not need every worker agent.** In a master/worker setup, keep the group centered on the master unless a worker must speak to the user directly.
- **Queueing is still needed internally.** Even if the user only talks to the master agent, the master still has to serialize work that targets the same worker or depends on earlier steps.
