...
Session-derived routing notes and spawnability pitfalls live in `references/multi-agent-routing.md`.
...
- A wrapper command (for example a `.bat` file on Windows or a shell script on macOS/Linux) is the stable handoff point to invoke the remote profile over SSH.
- For Windows targets, expect Windows-shell quoting rules; validate a tiny end-to-end message before wiring the routing into Kanban.
