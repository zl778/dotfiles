...
## 10. HeartMuLa Open-Source Music Generation

For generating full songs from lyrics + tags using the HeartMuLa open-source music model locally, see `references/heartmula-heartlib-setup.md`. This companion guide covers:

- Hardware requirements (8GB+ VRAM recommended) and GPU/CPU modes
- Installation from source with dependency fixes
- Lyrics and tag formatting for best results
- Key generation parameters (topk, temperature, cfg_scale)
- Patches required for newer library compatibility
