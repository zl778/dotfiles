...
metadata:
  hermes:
    tags: [creative-coding, generative-art, p5js, canvas, interactive, visualization, webgl, shaders, animation, pretext, typography]
    related_skills: [ascii-video, manim-video, excalidraw, architecture-diagram]
---
...
For creative browser demos using the `@chenglou/pretext` library (DOM-free text layout for ASCII art, typographic flow around obstacles, text-as-geometry, kinetic typography), see `references/pretext-demos.md`. Templates at `templates/pretext-hello-orb-flow.html` and `templates/pretext-donut-orbit.html`.

Key patterns covered:
- Text reflowing around a moving sprite (canvas, 60fps)
- Per-grapheme physics for typographic effects
- Text-as-geometry games (Breakout with word bricks)
- Single-file HTML demos, no build step required
