...
## Quick Sketch & Throwaway Variants

For the case where the user wants to **see a design direction before committing** — rapid exploration with 2-3 disposable HTML mockups for comparison, not polished artifacts. This is a lighter version of the full design pipeline.

### When to sketch instead of design

Load this mode when the user says "sketch this screen", "show me what X could look like", "compare layout A vs B", "give me 2-3 takes", "mockup this before I build". It means they want breadth over depth.

### The 3-step sketch loop

**1. Intake (3 questions, one at a time):**
...
Reflect each answer before the next question. Skip if already given.

**2. Build 2-3 variants in parallel:**
Each variant is a single self-contained HTML file with inline `<style>`, system fonts (or one Google Font), and realistic fake content — no lorem ipsum. Each variant takes a **different design stance**, not different pixel values.

Good axes to pull apart:
- Density (compact / airy)
- Emphasis (content-first / action-first / tool-first)
- Layout (single-column / sidebar / split-pane)

Tailwind via CDN is fine. At least one state transition per variant (open/close, filter, toggle). A frozen static image is worse than a sloppy interactive one.

**Default CSS reset for speed:**
```html
<style>
...
**3. Verify visually** using `browser_navigate` + `browser_vision` to catch layout bugs no source inspection reveals.

**4. Present head-to-head comparison** with your opinionated take on which is strongest for which use case.
