...
- `references/tui-gateway-python-resolution.md` — TUI gateway startup timeout ("Timed out connecting to Hermes backend after 15000ms"), Python interpreter resolution, and the HERMES_PYTHON fix.
...
6. **TUI gateway fails to start ("Timed out connecting to Hermes backend after 15000ms").** The Hermes desktop app (Hermes.app, Electron) spawns `python -m tui_gateway.entry` and waits 15s for `gateway.ready`. If the wrong Python is used (system `python3` instead of the Hermes venv), the process crashes immediately with `ModuleNotFoundError: No module named 'dotenv'`. Fix: set `HERMES_PYTHON` to the venv Python before launching. See `references/tui-gateway-python-resolution.md` for full diagnosis and the wrapper-script fix.
...
- When debugging TUI gateway startup issues, the Python interpreter used by GatewayClient is determined by `resolvePython()` in `gatewayClient.ts`, which checks `$HERMES_PYTHON` → `$PYTHON` → `$VIRTUAL_ENV/bin/python` → `root/.venv|venv/bin/python{3}` → system `python3`. The desktop app (Hermes.app) frequently misses the venv because `root` resolves inside the asar bundle. Always set `HERMES_PYTHON` explicitly when launching from the app bundle.
