...
4. Availability state
   - `running` means the gateway is already up.
   - `stopped` means the profile is not hot, but it can still be invoked on demand with `hermes -p <profile> chat -q ...`.

Keep availability state separate from capability: a stopped profile can still be a valid worker.
...
Record these fields in the worker registry /通讯录:
...
### 4) If the user wants to design a role roster
...
- one constant registry snapshot for current status /常驻-vs-按需 decisions
...
- When a profile is intentionally on-demand, do not treat `stopped` as an error; invoke it directly when needed
...
- `references/current-worker-registry.md` — current registry snapshot, running/stopped semantics, and the on-demand vs always-on split used in this session
