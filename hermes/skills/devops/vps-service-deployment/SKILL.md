...
**Primary path: NPM-first** (current setup on this VPS) — Add the service in Nginx Proxy Manager first, then deploy the container. NPM auto-manages SSL.
**Legacy path: system Nginx + acme.sh** — Used before NPM migration. Still valid for standalone setups but requires manual SSL management.

If NPM is already running on the VPS and you're adding a new service, use the **NPM-first** path below. If the VPS has system Nginx (no NPM), use the legacy path in the reference section.

---

## NPM-first deployment (primary)

### Step 0 — Pre-requisite: Cloudflare DNS

User adds an A record in Cloudflare **before** deploying:

| Type | Name | Target |
|:---:|:----|:------|
| A | `<SUBDOMAIN>` | `<VPS-IP>` |

Set to **gray cloud (DNS only)** — orange cloud breaks Let's Encrypt ACME challenges.
...
Verify: `ssh root@<VPS-IP> 'docker ps --format "table {{.Names}}\t{{.Status}}"'`

### Step 2 — Add NPM proxy host

1. Log into NPM at `https://npm.<DOMAIN>`
2. Click **Add Proxy Host**
3. **Details** tab:
   - Domain Names: `<SUBDOMAIN>.<DOMAIN>`
   - Scheme: `http`
   - Forward IP: get the container's **Docker bridge IP** (`docker inspect <container> --format '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}'`) — **not** `127.0.0.1`
   - Forward Port: use the **container's internal port** (not the host-mapped port)
   - Block Common Exploits: ✅
4. **SSL** tab:
   - Select **Request a new Certificate**
   - (Email must be set in NPM: Settings → Default Let's Encrypt Email)
5. Click **Save** — NPM auto-issues Let's Encrypt cert and sets up auto-renewal
...
## Alternative: system Nginx + acme.sh (legacy)

Use this path only if the VPS does NOT have Nginx Proxy Manager.
...
15. **Beszel — Hub first, then Agent** — Beszel has two components:
    ...
    Workflow: deploy Hub → open web UI → create admin account → add a system (Hub generates a key command) → deploy agent with the key.
    **If agent was deployed with wrong token**, delete the system from Hub and re-add to get a fresh token.

16. **docker-compose beats long docker run for complex env vars** — When a service has many environment variables with special characters (SSH keys, tokens, URLs), write a `docker-compose.yml` and `docker compose up -d` instead of a long `docker run -e` command. Avoids shell quoting and escaping issues entirely.

17. **Uptime Kuma vs Beszel — not redundant** — They serve complementary roles:
...
- `references/bitwarden-uri-matching.md` — Fixing credential suggestions across subdomains
