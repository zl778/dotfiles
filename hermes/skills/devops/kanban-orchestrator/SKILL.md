...
A **browser-based Kanban Dashboard** ships as a built-in plugin (`plugins/kanban/dashboard/`). It provides a drag-and-drop board with real-time WebSocket updates, per-task drawers, and recovery controls.

```bash
hermes dashboard   # Opens http://127.0.0.1:9119, then click the Kanban tab
```

For full details — what the plugin provides, the three access surfaces (web dashboard / CLI / in-chat), and the Desktop app limitation — see `references/kanban-dashboard-visual-ui.md`.
