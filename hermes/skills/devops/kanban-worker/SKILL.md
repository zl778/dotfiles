...
When a Kanban worker wants to run Codex CLI as an isolated implementation lane, use this pattern. Hermes always owns the Kanban lifecycle, reconciliation, testing, and handoff. Codex is an input lane only.

### When to use the Codex lane

Use when: coding/refactor/doc task with clear acceptance criteria, bounded diff evaluable in one run, and isolated worktree. Don't use when: task needs human judgment, touches secrets or production data, or a small direct edit is faster.

### Ownership rules

1. **Hermes owns the Kanban lifecycle.** Codex must never call `kanban_complete`, `kanban_block`, or `kanban_create`.
2. **Hermes owns final acceptance.** Treat Codex diffs as untrusted patches.
3. **Hermes owns test execution.** Repeat verification from Hermes.
4. **Hermes owns safety.** Reject lanes that change safety boundaries.
5. **Hermes owns cleanup.** Kill stuck Codex processes, remove temporary worktrees.
...
- `codex exec` — bounded one-shot edits, exits on its own
- Codex `/goal` — broader multi-step work with objective tracking (launch interactively with pty)

### Prompt construction

Every Codex prompt must include:
- task_id, title, full acceptance criteria
- repo path, worktree path, branch name
- "Hermes owns Kanban lifecycle; Codex is an input lane only"
- required output: summary, files changed, commits, tests run, known risks

### Reconciliation checklist (Hermes must run before accepting)

- [ ] `git status --short --branch` shows only expected files
- [ ] Diff reviewed by Hermes
- [ ] No secrets, credentials, or artifacts included
- [ ] Hermes ran canonical tests independently
- [ ] Accepted commits applied to Hermes-owned workspace

### Outcome metadata

Include `metadata.codex_lane` in `kanban_complete`:
```json
{
  "codex_lane": {
    "used": true,
...
For tasks that intentionally skip Codex, set `used: false`, `mode: "skipped"`, and explain in `rejected_reason`.

### Common pitfalls

1. Treating Codex self-report as verification — always inspect diff and rerun tests
2. Running Codex in shared dirty checkout — always isolate in worktree/branch
3. Accepting broad unrelated cleanup because tests pass — reject or cherry-pick
4. Using `/goal` for quick edits — prefer `codex exec`
5. Killing stuck lane without recording why — always document `rejected_reason`
