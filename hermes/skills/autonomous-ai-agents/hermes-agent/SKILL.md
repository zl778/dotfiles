...
metadata:
  hermes:
    tags: [hermes, setup, configuration, multi-agent, spawning, cli, gateway, development, skill-installation]
...
When a skill is NOT available in the Hermes skills hub (`hermes skills search` returns nothing) and you need to install from a GitHub repo or direct download URL, see `references/external-skill-installation.md`. It covers:

- Downloading skills from GitHub archives or raw SKILL.md URLs
- Detecting runtime (Python/Node.js/Bash) and writing `runtime.conf`
- Installing dependencies and verifying the skill works
- Configuring credentials when Hermes' secret redaction blocks `.env` writes
...
- `references/debugging-tui-slash-commands.md` — debugging TUI slash commands (Python registry → gateway → Ink frontend)
- `references/s6-container-supervision.md` — s6-overlay supervision tree in the Docker image
